...
def nanRemoval(datapath,inpath):
    ...
    dataz = data.read(1)
    dataz[dataz == 0] = np.nan
    #print("no of nans: ",dataz[np.isnan(dataz)] )
    
    if not ~np.isnan(dataz).all():
        logging.info(f'This scene only has NaNs: {datapath}')
    
    #print("no of zeros: ", dataz[dataz == 0]  )

    if len(dataz[np.isnan(dataz)]) > 0:
...
if __name__ == "__main__":       
    ...
    #area of interest
    if area == "Greenland":
        toProcess = gpd.read_file("Greenland_50kmTilesBuffer.geojson")
    ...
    #SH_CLIENT_ID = 'cc3c9643-6fd3-4d0f-a1a9-39fa4c6daa68'
    #SH_CLIENT_SECRET = '16f4ec34-2e6c-4688-b'
    ...
    #test = glob.glob(f'./{DL_FOLDER}/*/*/*.tif')
    """
    if len(test) > 300: 
        check = True
    else:
    ...
    if not check:
        ...
        logging.info("Data is already available, skipping download.....")
    ...
    misc_prod_out = ['threshold.tif','grain_diameter.tif','snow_specific_surface_area.tif','sza.tif','vza.tif','saa.tif','vaa.tif',\
                  'albedo_bb_planar_sw.tif','albedo_bb_spherical_sw.tif','isnow.tif','AOD_550.tif','ANG.tif','r0.tif','cloud_mask.tif',\
                    'al.tif','O3_SICE.tif','factor.tif','cv1.tif','cv2.tif']
    ...
    cloudpath = glob.glob(basefolder  + os.sep + DL_FOLDER + "/*/*/*SCDA.tif")
    ...
    dlpath = [DL_FOLDER for ii in range(len(sceneno))]
    ...
    with get_context("spawn").Pool(12) as p:     
            p.starmap(mask_sice,zip(sicePathsfilt,demPathsfilt,sceneno,usrpath,dlpath))
            p.close()
    ...
    #logging.info(f'First SICE file: {sicePathsfilt[0]}, Scene name: {sceneno[0]}, User Path: {usrpath[0]}')    
    ...
    if len(projection) > 0 and not projection == '4326' : 
        destFolder = f'{baseFolder}/transposed/{DATE_FOLDER}'
