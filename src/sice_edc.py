...
def pySICE_EDC(path,dempath,scene):
    #print("This Scene is being processed: ", scene)
    InputFolder = path + "/"
    demFolder = dempath + "/"

    # %% ================ input tif ================ %% #
       
    Oa01 = rio.open(InputFolder + 'toa1_filt.tif')
    meta = Oa01.meta
    ...
    for i in range(21):
        ...
        try:
            dat = rio.open((InputFolder + 'toa' + str(i + 1) + '_filt.tif')).read(1).astype('float32')
    ...
    ozone = rio.open(InputFolder + 'totalozone_filt.tif').read(1).astype('float32')
    #water = rio.open(InputFolder + 'WV.tif').read(1).astype('float32')
    sza = rio.open(InputFolder + 'sza_filt.tif').read(1).astype('float32')
    saa = rio.open(InputFolder + 'saa_filt.tif').read(1).astype('float32')
    vza = rio.open(InputFolder + 'vza_filt.tif').read(1).astype('float32')
    vaa = rio.open(InputFolder + 'vaa_filt.tif').read(1).astype('float32')
    height = rio.open(demFolder + 'response_filt.tif').read(1).astype('float32')
    ...
    try:
        ...
        print("size of isnow: ",np.size(isnow)) 
    ...
    #corruptscenes = ["29328","22309","26448","26633","20868","24815","26800","22853","22480","26268","26453","28424","25014",\
    #                "27342","23917","26806","28785","21946","26994","24817","21945","21769","28245","22489","29149","23026"]
    
    
    
    #if scene in corruptscenes:
    #    print("toa17 corr with ozone: ", np.nansum(toa_cor_o3[16, :, :]))
    #    print("toa21 corr with ozone: ", np.nansum(toa_cor_o3[20, :, :]))
    #    print("ak1: ", np.nansum(ak1))
    #    print("ak2: ", np.nansum(ak2))
    ...
    try:
        ...
        print("This scene is corrupted: ", scene)
        print(demFolder)
        print(InputFolder)
        
    
    
    #print(np.nanmean(tau))
    #print(np.nanmean(p))
    #print(np.nanmean(g))
    #print(np.nanmean(gaer))
    #print(np.nanmean(taumol))
    #print(np.nanmean(tauaer))
    ...
    #if scene in corruptscenes:
    #    print("diamter: ", np.nansum(D.ravel()))
    #    print("area: ", np.nansum(area))
    #    print("al: ", np.nansum(al))
    #    print("r0: ", np.nansum(r0))
    #    print("bal: ", np.nansum(bal))
        
    
    #if np.nansum(D.ravel()) == 0: 
    ...
    #print(np.nanmean(taumol))
    #print(np.nanmean(tauaer))
    #print(np.nanmean(am1))
    #print(np.nanmean(am2))
    #print(np.nanmean(amf))
    ...
    #print("Sum Zero :",np.nansum(D.ravel()))
